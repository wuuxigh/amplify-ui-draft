...
interface BlurHandler<T> {
    onBlur?: (event: T extends 'react' ? React.FocusEvent<HTMLInputElement> : T extends 'react-native' ? any : unknown) => void;
}
interface ChangeHandler<T> {
    onChange?: (event: T extends 'react' ? React.ChangeEvent<HTMLInputElement> : T extends 'react-native' ? any : unknown) => void;
}
interface ChangeTextHandler<T> {
    onChangeText?: T extends 'react-native' ? (event: string) => void : never;
}
export interface SubmitHandler<T> {
    onSubmit?: (e: T extends 'react' ? React.FormEvent<HTMLFormElement> : any) => void;
}
/*******************************/
/** Primitive component types **/
/*******************************/
type ButtonVariant = 'copy' | 'link' | 'outline' | 'primary' | 'secondary';
type ButtonType<V extends ButtonVariant> = V extends 'primary' ? 'submit' : 'button';
...
type ButtonGroupPrimitiveProps = {
    children?: React.ReactNode;
};
type ImagePrimitiveProps<T> = {
    children?: React.ReactNode;
} & ImgSrc<T>;
type MessagePrimitiveProps = {
    messageType?: 'info' | 'error';
};
type ViewPrimitiveProps = {
    children?: React.ReactNode;
};
type FieldsetPrimitive = {
    isDisabled?: boolean;
};
type FieldType = 'checkbox' | 'email' | 'password' | 'radio' | 'select' | 'tel' | 'text';
interface FieldPrimitivePropsBase<P, T> extends ChangeHandler<P>, ChangeTextHandler<P>, BlurHandler<P> {
    errorMessage?: boolean;
    hasError?: boolean;
    name: string;
    type?: T;
}
type FieldPrimitiveProps<P, T extends FieldType = FieldType> = FieldPrimitivePropsBase<P, T>;
...
/*****************************/
export type CompoundControlName = 'Actions' | 'Links' | 'SetupTotp' | 'FederatedProviders' | 'VerifyContactMethod';
...
export type ActionsControlType<T = unknown> = (() => JSX.Element) & ActionsPrimitives<T>;
interface LinksPrimitives<P> {
    Button: React.ComponentType<LinkButtonPrimitiveProps<P>>;
    ButtonGroup: React.ComponentType<ButtonGroupPrimitiveProps>;
}
export type LinksControlType<T = unknown> = (() => JSX.Element) & LinksPrimitives<T> & NonPrimitiveControls['Links'];
...
export type FederatedProvidersControlType<T = unknown> = (() => JSX.Element) & FederatedProvidersPrimitives<T> & NonPrimitiveControls['FederatedProviders'];
...
export type SetupTotpControlType<T = unknown> = (() => JSX.Element) & SetupTotpPrimitives<T>;
interface VerifyContactMethodPrimitives<T = unknown> {
    RadioGroup: React.ComponentType<RadioGroupPrimitiveProps<T>>;
}
export type VerifyContactMethodControlType<T = unknown> = (() => JSX.Element) & VerifyContactMethodPrimitives<T>;
/**
 ...
 */
export interface PrimitivesDefault<T = unknown> extends CompoundPrimitiveControls<T>, PrimitiveControls<T> {
}
/**
...
export type Components<T extends PrimitivesDefault> = {
    [K in keyof PickControls<T>]?: K extends CompoundControlName ? Partial<T[K]> : T[K];
};
/**
