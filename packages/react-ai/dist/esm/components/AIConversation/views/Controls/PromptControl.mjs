...
const PromptGroup = React__default.forwardRef(function ButtonGroup(props, ref) {
    const suggestedPromptsArray = React__default.useContext(SuggestedPromptsContext);
    const { setInput } = React__default.useContext(ConversationInputContext);
    if (!suggestedPromptsArray) {
        return;
    }
    return (React__default.createElement(PromptGroupBase, { ...props, ref: ref }, suggestedPromptsArray.map((prompt, index) => {
        return (React__default.createElement(PromptCard, { key: index, "aria-label": prompt.inputText, onClick: () => setInput &&
                setInput((prevInput) => ({
                    ...prevInput,
